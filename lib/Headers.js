        ...
        if (attributeName) {
            if (attributeValue) {
                ...
                var tokensAfterUpdate = [tokens[0]],
                    foldedParameters = rfc2231.encodeAndFoldParameters(parameters);

                Object.keys(foldedParameters).forEach(function (parameterName) {
                    tokensAfterUpdate.push(parameterName + '=' + foldedParameters[parameterName]);
                });
