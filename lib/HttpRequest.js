...
var RequestLine = require("./RequestLine");
...
var ownPropertyNames = ["requestLine"]
...
HttpRequest.prototype.populateFromString = function(str) {
  ...
  if (matchRequestLine) {
    ...
    var requestLineFragments = requestLineStr.split(" ");
  ...
  get: function() {
    ...
    if (typeof authorizationHeaderValue === "string") {
      ...
      if (
        ...
        var credentials = new Buffer(authorizationFragments[1], "base64")
          .toString("utf-8")
          .split(":");
        var username = credentials.shift();
        var password = credentials.join(":") || undefined;
        return {
          username: username,
          password: password
        };
...
HttpRequest.prototype._updateUrl = function(url, invokedAsSetter) {
  ...
  if (fragments.length > 0) {
    var matchUrl = fragments[0].match(
    ...
    if (matchUrl) {
      ...
      if (typeof auth === "string" && auth.length > 0) {
        ...
        var username = safeDecodeURIComponent(authFragments.shift());
        var password = safeDecodeURIComponent(authFragments.join(":"));
        this.headers.set(
          "Authorization",
          `Basic ${
            typeof Buffer !== "undefined"
              ? new Buffer(`${username}:${password}`, "utf-8").toString(
                  "base64"
                )
              : btoa(auth)
          }`
